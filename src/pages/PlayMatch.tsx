...
import { AlertCircle, Save, Trophy, X, ArrowLeft, CheckCircle, Plus, Users } from 'lucide-react';
    ...
    const fetchData = async () => {
      try {
        ...
        if (!predictionsSnapshot.empty) {
          ...
          // Map selected players to their slots
          const selectedPlayerIds = prediction.data().selectedPlayers;
          const selectedPlayersList = Array(5).fill(null);
          
          selectedPlayerIds.forEach((playerData: any, index: number) => {
            const player = playersWithTargets.find(p => p.id === playerData.id);
            if (player) {
